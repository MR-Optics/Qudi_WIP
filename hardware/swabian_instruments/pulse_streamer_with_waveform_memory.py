...
class PulseStreamer(Base, PulserInterface):
    ...
    _gated_counter_channel = ConfigOption('gated_counter_channel', 1, missing='warn')
    _uw_x_channel = ConfigOption('uw_x_channel', 2, missing='warn')
    _uw_list_trigger_channel = ConfigOption('uw_list_trigger_channel', 4, missing='warn')
    ...
    __generated_waveforms_list = StatusVar(name='generated_waveforms_list', default={})
    ...
    def __init__(self, config, **kwargs):
        ...
        self._laser_on_state = ps.OutputState([self._laser_channel], 1, 0)
        self._mw_on_state = ps.OutputState([self._uw_x_channel], 0, 0)
    ...
    def get_constraints(self):
        ...
        constraints.d_ch_high.min = 3.0
        constraints.d_ch_high.max = 3.0
        constraints.d_ch_high.step = 0.0
        constraints.d_ch_high.default = 3.0

        constraints.a_ch_amplitude.max = 2 # V
        constraints.a_ch_amplitude.min = 0
        constraints.a_ch_amplitude.step = 61e-6 # 14 bit resolution 1:16386
        constraints.a_ch_offset.min = -1
        constraints.a_ch_offset.max = 1
        constraints.a_ch_offset.step = 61e-6
        ...
        activation_config['all'] = frozenset({'a_ch1', 'a_ch2','d_ch1', 'd_ch2', 'd_ch3', 'd_ch4', 'd_ch5', 'd_ch6', 'd_ch7', 'd_ch8'})
    ...
    def pulser_on(self):
        ...
        """
        if self._seq:
            self.pulse_streamer.setTrigger(start=self._trigger)
            self.pulse_streamer.stream(self._seq, n_runs=self.n_runs)
    ...
    def load_waveform(self, load_dict):
        ...
        if waveform not in list(self.__generated_waveforms_list.keys()):
        ...
        self._seq = self.__generated_waveforms_list[waveform]


        return self.get_loaded_assets()[0]

    def load_waveform_from_memory(self, waveform_name):

        if waveform_name not in list(self.__generated_waveforms_list.keys()):
            self.log.error(f'Waveform {waveform_name} is not loaded')
            return -1

        waveform = self.__generated_waveforms_list[waveform_name]
    ...
    def clear_all(self):
        ...
        self.__generated_waveforms_list = dict()
    ...
    def get_analog_level(self, amplitude=None, offset=None):
        ...
        """
        return {'a_ch1': 2.0, 'a_ch2': 2.0}, {'a_ch1': 0.0, 'a_ch2': 0.0}
    ...
    def get_active_channels(self, ch=None):
        ...
        if len(ch) < 1:
            for chnl in range(1, 9):
                ch_dict['d_ch{0}'.format(chnl)] = True
            for chnl in range(1,3):
                ch_dict[f'a_ch{chnl}'] = True
        else:
            for channel in ch:
                ch_dict[channel] = True
        return ch_dict
    ...
    def write_waveform(self, name, analog_samples, digital_samples, is_first_chunk, is_last_chunk,
        ...
        active_channels_list = [chnl for chnl in activation_dict if activation_dict[chnl]]
        active_analog = natural_sort(chnl for chnl in active_channels if chnl.startswith('a'))
        active_digitial = natural_sort(chnl for chnl in active_channels if chnl.startswith('d'))
        ...
        if is_first_chunk:
            ...
            self._current_waveform = {chnl:[] for chnl in active_channels_list}
        ...
        for channel_number, samples in digital_samples.items():
            ...
            self._current_waveform[channel_number].extend(pulses)
        ...
        for channel_number, samples in analog_samples.items():
            ...
            for new_channel_index in range(1, new_channel_indices.size):
                pulse = [new_channel_indices[new_channel_index] - new_channel_indices[new_channel_index - 1],
                         samples[new_channel_indices[new_channel_index - 1] + 1].astype(np.float)]
                pulses.append(pulse)
        ...
        if is_last_chunk:
            created_seq = self.pulse_streamer.createSequence()
            for channel_number, pulse_pattern in self._current_waveform.items():
                if channel_number.startswith('d'):
                    swabian_channel_number = int(channel_number[-1]) - 1
                    created_seq.setDigital(swabian_channel_number, pulse_pattern)
                else:
                    swabian_channel_number = int(channel_number[-1]) - 1
                    created_seq.setAnalog(swabian_channel_number, pulse_pattern)

            self.__generated_waveforms_list[self._current_waveform_name] = created_seq


        return len(samples), [self._current_waveform_name]
    ...
    def get_waveform_names(self):
        ...
        """
        waveform_names = list(self.__generated_waveforms_list.keys())
    ...
    def turn_on_laser(self):
        ...
        @return:
        """
        if self._mw_state is True:
            self.pulse_streamer.constant(self._laser_mw_on_state)
        else:
            self.pulse_streamer.constant(self._laser_on_state)
    ...
    def turn_off_laser(self):
        ...
        """
        if self._mw_state is True:
            self.pulse_streamer.constant(self._mw_on_state)
        else:
            self.pulse_streamer.constant()
    ...
    def ask_are_you_done(self):
        while not self.pulse_streamer.hasFinished():
            time.sleep(0.02)
        self.pulser_done.set()


    def is_pulser_seq_done(self):
        self.pulser_done = threading.Event()
        thread = threading.Thread(target=self.ask_are_you_done)
        thread.start()
        self.pulser_done.wait()
        self.log.info('Pulse seq is done')

        return 0

    def constant_output(self, active_channels, analog_values):
        self.pulse_streamer.constant(ps.OutputState(active_channels, analog_values[0], analog_values[1]))
        return

    def change_laser_channel(self,channel):
        self._laser_channel = int(channel)
        self._laser_mw_on_state = ps.OutputState([self._laser_channel, self._uw_x_channel], 1.0, 0)
        self._laser_on_state = ps.OutputState([self._laser_channel], 1.0, 0)
        return 0


    def ODMR_sequence(self, frequency, omdr_line_length):
        """
        Generating the pulse sequence for the ODMR
        ...
        High = 1
        Low = 0
        pulse_period = int(1 / frequency * 1e9)
        full_pulse_duration = pulse_period * (omdr_line_length + 2)
        self._current_waveform = {key: [] for key in [self._laser_channel, self._uw_x_channel,
                                                       self._gated_counter_channel,
                                                       self._uw_list_trigger_channel]}

        low_pulse = [pulse_period/2, Low]
        high_pulse = [pulse_period / 2, High]

        pulses = []
        for i in range(omdr_line_length + 1):
            pulses.append(low_pulse)
            pulses.append(high_pulse)

        pulse = [pulse_period, Low]
        pulses.append(pulse)

        trigger_pulse = pulses.copy()
        trigger_pulse[1] = low_pulse
        
        self._current_waveform[self._gated_counter_channel].extend(pulses)
        self._current_waveform[self._uw_list_trigger_channel].extend(trigger_pulse)
        self._current_waveform[self._uw_x_channel].extend([(full_pulse_duration, High)])
        self._current_waveform[self._laser_channel].extend([(full_pulse_duration, High)])

        created_seq = self.pulse_streamer.createSequence()
        for channel_number, pulse_pattern in self._current_waveform.items():
            swabian_channel_number = int(channel_number)
            created_seq.setDigital(swabian_channel_number, pulse_pattern)
            if int(channel_number) == 4:
                created_seq.setAnalog(channel_number, [(full_pulse_duration, 1.0)])

        self.__generated_waveforms_list['ODMR_sequence'] = created_seq
        self.load_waveform_from_memory('ODMR_sequence')

        ...
        for step_no, seq_step in enumerate(sequence):
            ...
            if sequence.is_finite:
                for chnl in digital_channels:
                    ...
                    # TODO: temp fix this to aviod memery error (changed range(reps) to range(1))
                    ...
                    while element_samples_written != element_length_bins:
                        ...
                        try:
                            for chnl in ensemble_info['analog_channels']:
                                analog_samples[chnl] = np.empty(samples_to_add, dtype='float32')
                            for chnl in ensemble_info['digital_channels']:
                                digital_samples[chnl] = np.empty(samples_to_add, dtype=bool)
                        except MemoryError:
                        ...
                        for chnl in digital_high:
                            digital_samples[chnl][:] = digital_high[chnl]
                        for chnl in pulse_function:
                            analog_samples[chnl] = pulse_function[chnl].get_samples(time_arr) / (self.__analog_levels[0][chnl] / 2)
                        ...
                        if array_write_index == samples_to_add:
                            ...
                            if written_samples != samples_to_add:
                            ...
                            if samples_to_add > ensemble_info['number_of_samples'] - processed_samples:
                                samples_to_add = ensemble_info['number_of_samples'] - processed_samples
                                analog_samples = dict()
                                digital_samples = dict()
                                for chnl in ensemble_info['analog_channels']:
                                    analog_samples[chnl] = np.empty(samples_to_add, dtype='float32')
